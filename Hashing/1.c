...
void display() {
    int i;
    printf("\nIndex\tID\tName\t\tDepartment\tDesignation\n");
    for(i=0; i<MAX; i++) {
        printf("%d\t%d\t%s\t\t%s\t\t%s\n", i, hashtable[i].id, hashtable[i].name,hashtable[i].department, hashtable[i].designation);
    }
}
int search(int id) {
    int index = id % MAX;
        ...
        switch(choice) {
            ...
            case 2: {
                ...
                if(index == -1) {
                    ...
                    hashtable[index].department, hashtable[index].designation);
                }
                break;
            }
            case 3: {
